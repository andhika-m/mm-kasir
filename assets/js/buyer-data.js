...
function loadBuyers(page_number, total_row_displayed, searchVal) {
	let row_number;
	if (page_number < 2) {
		row_number = 0;
	} else {
				...
				(err, res) => {
					...
					if (res.row_number < 1) {
						...
						dialog.showMessageBoxSync({
							...
							type: "info",
							message:
								"Nama Buyer / Customer sudah ada dalam database, silahkan gunakan nama lain",
						});
					}
				}
			);
...
editBuyerData = (id) => {
	...
	db.all(query, (err, result) => {
		...
		editForm = `<div class="input-group mb-3">
                ...
                <button class="btn btn-sm btn-primary" onclick="submitEditBuyerData(${id})"><i class="fa fa-paper-plane"></i> Submit</button>`;
